...
export default function CalculatorTable({ initialOrders }: { initialOrders: any[] }) {
  const [orders, setOrders] = useState(initialOrders);
  const [editing, setEditing] = useState<{ orderNumber: string; field: string } | null>(null);
  const [editValue, setEditValue] = useState('');

  const startEdit = (orderNumber: string, field: string, currentValue: number) => {
  ...
  const saveEdit = async () => {
    ...
    try {
      await fetch(`/api/admin/orders/${editing.orderNumber.replace('#', '')}/costs`, {
        method: 'PUT',
      ...
      setOrders(orders.map(o => o.orderNumber === editing.orderNumber ? updatedOrder : o));
    } catch (error) {
  ...
  const totalStats = orders.reduce((acc, order) => ({
    ...
    <div>
      <div className={styles.statsRow}>
        <div className={styles.statMini}>
          ...
          <div className={styles.statMiniValue}>{totalStats.totalRevenue} zł</div>
        </div>
        <div className={styles.statMini}>
        ...
        <div className={styles.statMini}>
          ...
          <div className={`${styles.statMiniValue} ${totalStats.totalProfit >= 0 ? styles.positive : styles.negative}`}>
            ...
            {orders.map(order => {
              ...
              return (
                <tr key={order.orderNumber}>
                  ...
                  <td>{order.date}</td>
                  <td>
                    {editing?.orderNumber === order.orderNumber && editing?.field === 'costPrice' ? (
                      <input
                        type="number"
                        value={editValue}
                      ...
                      <div 
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          startEdit(order.orderNumber, 'costPrice', order.costPrice);
                        }}
                      >
                  ...
                  <td>
                    {editing?.orderNumber === order.orderNumber && editing?.field === 'deliveryCost' ? (
                      <input
                        type="number"
                        value={editValue}
                        onChange={(e) => setEditValue(e.target.value)}
                      ...
                      <div 
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          startEdit(order.orderNumber, 'deliveryCost', order.deliveryCost);
                        }}
                      >
